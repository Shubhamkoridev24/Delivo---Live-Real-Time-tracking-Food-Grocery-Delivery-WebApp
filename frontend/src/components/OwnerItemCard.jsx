...
function OwnerItemCard({data}) {
  ...
  try {
    await axios.get(`${serverUrl}/api/item/delete/${data._id}`, { withCredentials: true });
    // refetch shop
...
<div className="flex items-center gap-2">
    <div className='p-2  cursor-pointer rounded-full hover:bg-[#ff4d2d]/10 items text-[#ff4d2d]' 
    onClick={()=>navigate(`/edit-item/${data._id}`)}>
                        <FaPen size={16} />

    </div>

    <div className='p-2  cursor-pointer rounded-full hover:bg-[#ff4d2d]/10 items text-[#ff4d2d]' onClick={handleDelete}>
<FaTrashAlt size={16} />
    </div>
</div>
