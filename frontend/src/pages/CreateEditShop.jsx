...
function CreateEditShop() {
    ...
    const { myShopData: reduxShopData } = useSelector(state => state.owner)
    const { currentCity, currentState, currentAddress } = useSelector(state => state.user)

    const [myShopData, setLocalShopData] = useState(reduxShopData || null) // ← fix
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setLocalShopData(result.data) // ← heading update ke liye
            setLoading(false)
            navigate("/")
        } catch (error) {
            console.log(error)
    ...
    useEffect(() => {
        setLocalShopData(reduxShopData)
    }, [reduxShopData]) // ← redux changes pe local update
            ...
            <div className='max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-orange-100'>
                <div className='flex flex-col items-center mb-6'>
                    ...
                    <div className='text-3xl font-extrabold text-gray-900'>
                        {myShopData ? "Edit Shop" : "Add Shop"} {/* ← fix */}
                    </div>
                </div>
